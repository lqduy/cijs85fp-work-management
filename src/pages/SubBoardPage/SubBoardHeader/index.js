...
const SubBoardHeader = ({ boardData, onSetVerticalView }) => {
    ...
    <div className={cx('wrapper')}>
      <div className={cx('left-part')}>
        <input
          ...
          className={cx('board-title', {
            'board-title__editing':
              editingBoardTitle && boardTitleValue.length > 0,
      ...
      <div className={cx('right-part')}>
        <div className={cx('setting-item__wrap')}>
          <Button
            leftIcon={filterIcon}
            className={cx('setting-item', 'filter-btn')}
          >
            Filter
          </Button>
        </div>
        <div className={cx('setting-item__wrap')}>
          <Button
            leftIcon={viewModeIcon}
            className={cx('setting-item')}
            onClick={onSetVerticalView}
          >
            View mode
          </Button>
        </div>
        <div className={cx('setting-item__wrap')}>
          <Button
            onClick={() => setOpenSubMenu(SUBBOARD_HEADER_SUBMENU.SETTING)}
          >
            {threeDotsIcon}
          </Button>
              ...
              <div className={cx('setting-item', 'remove-board')}>
                ...
                {removePreparing && (
                  <div className={cx('preparing-remove-btn')}>
                    <Button
                      className={cx('submit-remove-btn')}
